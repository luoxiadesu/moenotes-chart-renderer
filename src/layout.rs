...
impl Layout {
    pub fn build(score: &Score, mut options: Options) -> Result<Self> {
            ...
            for (i, a) in notes.iter().enumerate() {
                for b in notes.iter().skip(i + 1) {
                    ...
                    if delta > 0 && a.left < b.right && b.left < a.right {
                        spacing = spacing.max((options.note_height + 3.) * 480. / delta as f64);
                    }
                }
            }
        ...
        let height = (HEADER + PAD * 2. + track_height + FOOTER).ceil();
            ...
            height <= 32768.,
            "Image too tall; reduce scale or use columns"
        );
        let used =
    ...
    pub fn page_width(&self, n: usize) -> Result<i32> {
        let w = (n as f64 * self.column_width + GAP).ceil();
        let pixels = w * self.height as f64;
        ensure!(
            w <= 32768. && pixels <= 256_000_000.,
            "Full sheet exceeds 64 million pixels; lower scale"
        );
        ensure!(
            pixels * (self.options.supersample * self.options.supersample) as f64 <= 256_000_000.,
            "Supersampled full sheet exceeds 256 million pixels; lower --supersample or scale"
        );
        Ok(w as i32)
    }
}
